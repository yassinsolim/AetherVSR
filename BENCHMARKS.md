...
- Chromium quantises WebGPU timestamp values for fingerprinting resistance.
  Individual samples are coarse; aggregate over hundreds of frames. The initial
  Milestone 1 runs use n=240 samples; later sections state their own windows.
...
## Environment for the initial Milestone 1 runs
...
**Scope for the Milestone 1 tables in this section:** each `GPU upscale` figure brackets the
...
## Milestone 10: extension MVP measurements

**Verdict: EXTENSION MVP PARTIAL, not complete or READY.** These extension
measurements are separate from M9's harness results above. The long run passed
duration, FPS and no-fallback checks but failed the absolute 1% combined-loss
gate. Relative overhead allowances do not excuse that failure. Whole extension
reload remains unverified, and only one public player attached successfully.
Acceptance and frozen tolerances are recorded in
[docs/M10-PREREGISTRATION.md](docs/M10-PREREGISTRATION.md) and
[docs/M10-CALIBRATION-PLAN.md](docs/M10-CALIBRATION-PLAN.md).

### M10 environment and identity

| Field | Recorded value |
|---|---|
| Machine | Mac17,2, Apple M5 MacBook Pro, 24 GB unified memory |
| OS | macOS 26.6.2 |
| Browser | Chrome for Testing 153.0.8010.12 |
| GPU timestamps | Available; default browser quantization, not M9's Dawn overrides; no unsafe WebGPU or web-security override |
| Window / power | 1200x820 viewport, headful foreground, AC power |
| Display refresh rate | **not measured**; no earlier milestone's refresh measurement is imported |
| Source | Exact-CFR 1280x720 H.264/MP4, 60/1 fps; SHA256 `8d81acbe164da1d62b7d0d02a3cc66915c96e8aa90d45cac34d818fc33df1d4a` |
| Output / import | 2560x1440, exact 2x; production external-texture import |
| Model | Unchanged C16D2, 6,291 parameters; SHA256 `d76fae7a295cdcdaecb44e39f8c87ff68a59ca1e07fc7cfc347d252d3cad358a` |
| Production payload | 263,008 bytes; SHA256 `724507ec7d3e7a8c9a4ce7bf0fc772f2d2de9f56c1218693db52d6cba195258a` |
| Warmup / windows | Five seconds after actual runtime readiness; separate 30-second control/comparator windows and one >=600-active-second Auto window |

GPU whole-stage timestamps bracket neural ingest through the final blit. They
exclude decode, CPU import/command recording/submit, pre-span queue wait,
browser-internal import work and browser composition/presentation. Quantiles
use available raw samples whose submission time falls in the measured window,
not trailing 240-sample aggregates. Pending readbacks drain afterward without
extending the rate denominator. Missing timestamp samples are not zero-cost
frames. The harness and extension have different CSS/composition workloads;
this is not an isolated causal comparison of adapter cost.

### Final ten-minute Auto window
...
| Combined loss | **4.8011814863%; FAIL <=1% absolute gate** |
| Runtime tier | Neural throughout |
| Tier changes / owner changes / errors | 0 / 0 / 0 |
| Rewarm state records | 299; lifecycle evidence rewarming, not fallbacks or tier oscillation |
| Whole-stage GPU samples | 34,862 available samples for 35,011 rendered frames |
| Whole-stage GPU p50 / p95 | 6.488064 / 7.667712 ms, over those available whole-window samples |
| Core CPU p50 / p95 | 0.1 / 0.2 ms per measured frame, import/encode/submit bracket |
| Shared driver CPU p50 / p95 | Below timer resolution / 0.1 ms per measured callback; not zero CPU cost |
| Discovery + geometry | 0.209333 ms per playback second, aggregate owned work over the long window; excludes other adapter and browser work |

The registered combined-loss calculation is
`100 * (skips + decoder drops) / presented frames`, here
`100 * (876 + 847) / 35887`. These are distinct counters, not a measurement of
mutually exclusive lost frames. Neither counter is subtracted from the other.
CPU brackets are not GPU execution time or whole-renderer CPU; do not add nested
attachment callbacks to driver time, or sum overlapping CPU brackets. Below
resolution is not a measured zero.

| Long-run segment | Rendered fps | Whole-stage GPU p50 / p95 (ms) |
|---|---:|---:|
| First 120 seconds | 58.641667 | 6.88128 / 7.86432 |
| Last 120 seconds | 57.858333 | 6.029312 / 6.946816 |

Segment rates and GPU quantiles apply only to their respective 120-second
windows, not to the full-run sample population. The last-window rate is below
58 fps even though the full-window rate passes. GPU times fell while rendered
rate also fell; no causal or thermal attribution follows. Temperature, energy,
actual GPU residency and whole-renderer CPU are **not measured**.

### Final short Auto comparison

Each row is its own 30-second window after the same readiness warmup. Rates and
loss cover the full window; GPU quantiles use that window's available raw
whole-stage neural samples: 1,758 harness and 1,737 extension samples. The first
and last 120-second long-run segments contain 7,007 and 6,914 GPU samples.

| Consumer | Rendered fps | Combined loss | GPU p50 (ms) | GPU p95 (ms) |
|---|---:|---:|---:|---:|
| Standalone harness Auto | 58.86352728 | 3.44635909% | 5.373952 | 6.291456 |
| Production extension Auto | 58.16414622 | 5.13392857% | 5.89824 | 7.012352 |

The paired GPU comparison **PASSes** the frozen extension-minus-harness
allowances of **0.6 ms p50 / 1.3 ms p95**. Both rows still exceed the 1%
absolute-loss reference. The relative pass is not an absolute performance pass,
nor proof of the cause of loss. No-extension, installed-idle and activated
Baseline controls are separate evidence arms. Inactive arms have no upscaler
processing rate or GPU time: those metrics are **not measured**, not zero.

### Functional integration and parity

| Surface | Recorded outcome |
|---|---|
| Native production journeys | 40 PASS |
| Diagnostic journeys | 44 PASS; not substitutes for production acceptance |
| Whole extension reload | **UNVERIFIED**, native action timeout; not counted as a pass |
| Public Shaka clear-content player | **PASS**, 180.026923 s; seek, pause, resizes and container fullscreen |
| Shaka scroll down/back | **N/A**, no page scroll extent; not an exercised scroll pass |
| Public Plyr / Video.js | **NEGATIVE geometry**; one successful public player imposes a PARTIAL ceiling |
| Production external-import parity | 3/3 exact input and output RGBA hashes against the same harness within this route |
| Test sampled-import parity | 3/3 exact input and output RGBA hashes against the same harness within this route |

Parity is paused, instrumented replay with matched input/model/import route,
not a cross-route equivalence claim, SR-quality measurement or real-time
benchmark. Resource/control mocks establish contracts, not hardware pixel
evidence. The public-player results establish only tested integration, not
objective image improvement on unknown ground truth.

The supported adapter scope is the top document plus accessible open shadow
roots, not iframe or closed-shadow ownership. Native controls and showing
native tracks are unsupported; PiP/direct-video fullscreen preserve original
video. Page-owned playback, DOM and styles stay authoritative and unmodified.
The production permission set is `activeTab`, `scripting`, `storage`, with no
permanent hosts, web-accessible resources or MAIN-world execution. Activation
is per document; only the chosen mode persists per origin.

### Final evidence archive

Final capture archive, indexed by the closure report:

```text
results/m10-evidence.json
results/m10-final-performance-02.json.gz
results/m10-final-performance-02.bare.json.gz
results/m10-final-performance-02.idle.json.gz
results/m10-final-performance-02.base.json.gz
results/m10-final-performance-02.harness.json.gz
results/m10-final-performance-02.auto.json.gz
results/m10-final-performance-05.json.gz
results/m10-final-performance-05.long.json.gz
```

The manifest indexes provenance; the `02` captures hold the final short controls
and paired consumers, and `05.long` holds the sustained Auto evidence. See
[docs/M10-REPORT.md](docs/M10-REPORT.md) for acceptance, clean-clone checks and
publication status.
M11 cross-vendor validation is recommended only after the absolute-loss and
