...
function harness(options: Partial<AttachmentOptions> = {}) {
  ...
  const listeners = [video, document, document.defaultView].map(target => ({
...
describe('VideoAttachment', () => {
  ...
  it('uses the pipeline exception rather than a later device notification when both exist', async () => {
    const { attachment, pipeline, failure } = harness();
    await attachment.start();
    pipeline.error = { name: 'SecurityError', message: 'cross realm exception' };
    vi.mocked(watchDeviceFailures).mock.lastCall![1]('uncaptured error');
    expect(failure.mock.calls[0]![0]).toBe('cors-blocked');
  });
