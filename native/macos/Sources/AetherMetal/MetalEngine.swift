...
public final class MetalEngine {
    ...
    public init(model: ProductionModel, extent: Extent, precision: Precision) throws {
        try require(precision == .f32, "Only f32 is qualified by this implementation")
        ...
        for (name, values) in model.weights {
            let buffer = try allocate(values.count * 4)
        ...
        for name in ["stem.conv", "stem", "body.0.conv", "body.0", "body.1.conv", "body.1"] {
            buffers[name] = try allocate(extent.pixels * 16 * 4)
        }
        buffers["nearest.features"] = try allocate(extent.pixels * 4 * 16 * 4)
        for name in ["head", "nearest.input", "residual", "final"] {
    ...
    private func encode(_ name: String, command: MTLCommandBuffer) throws {
        ...
        case "stem.conv", "body.0.conv", "body.1.conv", "head":
            ...
            let source = stem ? "input" : head ? "nearest.features" : name == "body.0.conv" ? "stem" : "body.0"
            let layer = name.replacingOccurrences(of: ".conv", with: "")
            kernel = "convolution32"
        ...
        case "residual":
            kernel = "residual32"; count = UInt32(pixels * 12); length = Int(count)
    ...
    public func capture() throws -> TensorCapture {
        ...
        let checkpoints = ["stem", "body.0", "body.1", "final"].map { name -> StageTensor in
            ...
            let pointer = buffers[name]!.contents().bindMemory(to: Float.self, capacity: width * height * channels)
            return StageTensor(name: name, width: width, height: height, channels: channels,
                               values: Array(UnsafeBufferPointer(start: pointer, count: width * height * channels)))
        }
        var rgba = Data()
